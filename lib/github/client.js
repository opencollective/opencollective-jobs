...
const Client = stampit({
  ...
  methods: {
    ...
    reposForOrg (opts) {
      opts = opts || {};
      this.log.verbose(opts.org, `Getting repos for org "${opts.org}"`);
    ...
    contributorsInOrg (opts) {
      ...
      const data = {};
      this.pushLogger(this.log.newGroup('contrib'));
      this.pushLogger(this.log.newItem('orgs', orgNames.length));
      return Promise.map(orgNames, org => {
        data[org] = {};
        this.log.verbose(org, `Fetching contributors to repos in org "${org}"`);
        ...
        .tap(repos => {
          this.popLogger();
          this.pushLogger(this.log.newItem('repos', repos.length));
        })
        .map(repo => {
          const contributors = {};
          const repoData = {
          ...
          return this.contributorsForRepo(repo.full_name)
            .then(contributions => {
              ...
              data[repo.owner.login][repo.name] = repoData;
            })
            .tap(() => {
              this.log.completeWork(1);
            });
    ...
    orgMemberContributions(opts)
    {
      ...
      return Promise.map(options.orgs, org => {
        this.log.verbose(org, `Fetching EXTERNAL contributions of members`);
        return this.membersOfOrg({
          org: org,
          private: options.private
        })
          .tap(members => {
            this.log.verbose(org,
  ...
  .init(function initLogger (context) {
    ...
    this.log.debug('client', 'Created GitHub client with options %j',
      _.head(context.args));
  })
