  ...
  methods: {
    ...
    paginate: Promise.coroutine(function* (opts, reposByUser) {
      ...
      yield this.activity.getEventsForUserAsync(opts)
        ...
        .filter(
          event => event.repo.name.split('/')[0] !== opts.org.toLowerCase())
        .map(event => event.repo)
        .then(repos => _.uniqBy('full_name', repos))
        .filter(repo => !uniqueRepos.has(repo.name))
        .each(repo => {
          ...
          return this.getRepo({
            ...
            .catch(err => {
              if (JSON.parse(err).message === 'Not Found') {
                this.log.warn(`Repo ${repo.name} not found; it may have been removed`);
              } else {
