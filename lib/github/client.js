      ...
      return Promise.map(options.orgs, org => {
        ...
        return this.membersOfOrg({
          ...
          .then(events => {
            ...
            const membersByRepo = new Map();
            _.forEach(event => {
              const repoName = event.repo.name;
              const members = membersByRepo.has(repoName) ? membersByRepo.get(
                repoName) : new Map();
              const count = Number(members.get(event.actor.login) || 0) + 1;
              members.set(event.actor.login, count);
              membersByRepo.set(repoName, members);
            }, events);
            ...
            return Promise.map(uniqueRepos, repoName => this.getRepo(repoName)
              ...
              .then(repo => {
                ...
                if (repo.source) {
                  ...
                  if (user.toLowerCase() === org.toLowerCase()) {
                    this.log.verbose(`Repo "${repoName}" is a fork of an org project ("${sourceRepoName}"); discarding`);
                    membersByRepo.delete(repoName);
                    return null;
                  }
                  if (uniqueRepos.has(repo.source)) {
                    this.log.verbose(`Repo "${repoName}" is a fork of a non-org project ("${sourceRepoName}"); continuing`);
                    return null;
                  }
                  this.log.verbose(`Repo "${repoName}" is a fork of a non-org project ("${sourceRepoName}"); getting more info`);
                  return this.getRepo(sourceRepoName)
                    .catch(notFound(sourceRepoName))
                    .tap(repo => {
                      if (repo) {
                        membersByRepo.set(sourceRepoName,
                          membersByRepo.get(repoName));
                      }
                      membersByRepo.delete(repoName);
                    });
                }
                ...
                if (user.toLowerCase() === org.toLowerCase()) {
                  this.log.verbose(`Repo "${repoName}" is a an org project; discarding`);
                  membersByRepo.delete(repoName);
                  return null;
                }
                return repo;
              ...
              .then(_.compact)
              .each(repo => {
                repo.contributors = _.fromPairs(Array.from(membersByRepo.get(repo.full_name)));
              ...
              .then(_.omit('full_name'))
              .tap(repos => {
                this.log.verbose('github:client:org-member-contributions',
                  `Member(s) of org "${org}" contributed to ${repos.length} unique repo(s).`);
              });
          });
      }, {concurrency: 1});
    },
